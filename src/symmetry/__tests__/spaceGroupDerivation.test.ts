...
test('every stored field is what the operations say it is', () => {
  const wrong: string[] = [];
...
test('the unique axis and the origin choice re-derive', () => {
  for (const setting of SPACE_GROUP_SETTINGS) {
    const operations = spaceGroupOperations(setting);
...
test('the centring letter derives, and only a rhombohedral R setting differs', () => {
  const differ: string[] = [];
  for (const setting of SPACE_GROUP_SETTINGS) {
...
test('all 7244 operations print back byte for byte', () => {
  let checked = 0;
  for (const setting of SPACE_GROUP_SETTINGS) {
    const operations = spaceGroupOperations(setting);
